...
public class DataStorage {
    ...
    static List<String> abbrOfBookList = new ArrayList<>();
    static List<String> abbrOfUserList = new ArrayList<>();
    static List<String> abbrOfAuthorList = new ArrayList<>();
...
//        userReadBookQueryBuilder.deleteCharAt(userReadBookQueryBuilder.length() - 2);
    ...
    static void createAllBookCreationQuery() {
//        StringBuilder returnQueryText = new StringBuilder();
//        returnQueryText.append("RETURN ");
        for (int i = 0; i < bookName.length; i++) {
            String createBookText = "CREATE (b" + i + ":Book{name:\"" + bookName[i] + "\",totalRead:" + getRandomInt(100) + ",point:" + getRandomDoublePoint() + "})";
            String abbr = "b" + i;
//            returnQueryText.append(abbr + ",");
            abbrOfBookList.add(abbr);
            System.out.println(createBookText);
        }
//        returnQueryText.deleteCharAt(returnQueryText.length() - 1);
//        System.out.println(returnQueryText);
//        abbrOfBookList.forEach(System.out::println);


    }

    static void createAllUserCreationQuery() {
...
//        returnQueryText.append("RETURN ");
        ...
        for (int i = 0; i < userNumber; i++) {
            String createUserText = "CREATE (u" + i + ":User{name:\"" + getRandomGenderName() + "\",lastname:\"" + getRandomGenderName() + "\",username:\"" + "user" + i + "\",password:\"pass\",totalFollowers:" + getRandomFollowers() + "})";
...
//        returnQueryText.deleteCharAt(returnQueryText.length() - 1);
        System.out.println("");
        StringBuilder createFollowingAction = new StringBuilder();
        createFollowingAction.append("CREATE ");
        for (int i = 0; i < userNumber; i++) {
            int userIndex = i;
            int totalFollowingPeople = getRandomInt(userNumber);
...
//    }
    ...
    /*
     *
     * create (n:User{name:"Ahmet",lastname:"SAGLIK",username:"ahmet",password:"emin",totalFollowers:10}) return n
     *
     * */

    static String getRandomGenderName() {
        if (random.nextInt(100) > 50) {
            return maleName[getRandomInt(maleName.length)];
        } else {
            return femaleName[getRandomInt(femaleName.length)];
        }
    ...
    public static int getRandomFollowers() {
